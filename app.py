...
async function loadPoints(){
  try{
    ...
    if(data.ok){
      ...
      if(pausedBanner) pausedBanner.style.display = _programPaused ? '' : 'none';
...
function renderRewardsList(){
  ...
  let html = '';
  if(_programPaused){
    html += `<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:10px 12px;margin-bottom:10px;font-size:12px;color:#991b1b">⏸️ Pansamantalang naka-pause ang Points Rewards Program - hindi muna pwede mag-redeem. Ligtas at buo pa rin ang points mo.</div>`;

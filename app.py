...
<div class="card" style="border:2px solid #f59e0b;background:#fffbeb">
  ...
  <div style="font-size:10px;color:#92400e;background:#fef3c7;padding:8px;border-radius:8px;margin-bottom:10px">🔒 Safe dito boss! Dito lang lilitaw OTP, hindi sa Sales. Ikaw lang makakakita.</div>
...
<div class="card"><input type="text" id="search" placeholder="Search store or phone..." oninput="loadCustomers()"><div style="font-size:10px;color:#666;margin-top:6px" id="customerCount">Loading...</div></div>
<div class="card"><div style="overflow-x:auto"><table><thead><tr><th>Store</th><th>Phone / Login</th><th>OTP / Status</th><th>Action</th></tr></thead><tbody id="tbody"><tr><td colspan=4 style="text-align:center;padding:20px;color:#999">Loading customers...</td></tr></tbody></table></div></div>
...
<label>New Password</label><input id="editPassword" type="text" placeholder="Set new password">
...
async function loadCustomers(){
  try{
    ...
    if(!rows.length){document.getElementById('tbody').innerHTML='<tr><td colspan=4 style="text-align:center;padding:20px;color:#888">No customers yet.</td></tr>'; return;}
    ...
    document.getElementById('tbody').innerHTML=filtered.map(r=>{
      ...
      return `<tr><td><b>${r.store_name}</b><br><small>₱${r.credit_balance||0}</small><br><button class="btn btn-qr" style="padding:4px 8px;font-size:10px;margin-top:4px" onclick="openEdit('${r.id}','${safeStore}','${r.phone}');setTimeout(generateQR,500)">📱 QR</button></td><td>${r.phone}<br><small style="color:${r.password_hash?'green':'red'}">${r.password_hash?'Has pwd':'No pwd'}</small></td><td>${otpInfo?'<span style="background:#fef3c7;padding:2px 6px;border-radius:10px;font-size:10px">OTP:'+otpInfo+'</span>':'-'}<br><small>${r.status||'active'}</small></td><td><button class="btn" style="background:#22c55e;color:#fff" onclick="openEdit('${r.id}','${safeStore}','${r.phone}')">Edit / QR</button></td></tr>`;
    }).join('');
  }catch(e){
...
async function savePassword(){
  ...
  const data=await res.json();
  document.getElementById('editStatus').textContent=data.ok?'✅ Saved! Now you can generate QR':'Error: '+(data.error||'');
  if(data.ok)loadCustomers();
}
async function generateQR(){
  if(!editingId){alert('Select customer first - pindutin Edit / QR muna');return;}
  ...
  try{
    ...
    document.getElementById('editStatus').textContent='✅ QR Generated! I-send sa customer';
...
async function loadOTPs(){
  try{
    ...
    document.getElementById('otpList').innerHTML=otps.map(o=>{
      ...
      return `<div style="background:${bg};border:1px solid ${border};border-radius:10px;padding:10px;font-size:12px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px"><b style="font-size:13px">📱 ${o.phone}</b><span style="font-size:9px;padding:2px 6px;border-radius:8px;background:#fff;border:1px solid ${border}">${isUsed} • ${isExpired}</span></div><div style="display:flex;align-items:center;gap:8px;margin:6px 0;flex-wrap:wrap"><div style="font-size:20px;font-weight:800;letter-spacing:4px;background:#fff;padding:6px 12px;border-radius:8px;border:2px dashed #f59e0b">${o.otp}</div><button class="btn btn-otp" onclick="navigator.clipboard.writeText('${o.otp}').then(()=>alert('Copied: ${o.otp}'))">📋 Copy</button><button class="btn" style="background:#fff;border:1px solid #f59e0b;color:#92400e" onclick="navigator.clipboard.writeText('Omega Ice OTP mo: ${o.otp}. Valid 5 mins. - ISESMO').then(()=>alert('Message copied for ${o.phone}'))">💬 Msg</button></div><div style="font-size:10px;color:#666;display:flex;justify-content:space-between"><span>⏰ ${o.created_at}</span><span>Exp: ${o.expires_at}</span></div>${o.reseller_name?`<div style="font-size:10px;color:#00609C;margin-top:4px">👤 ${o.reseller_name}</div>`:''}</div>`;
    }).join('');
  }catch(e){
...
def api_customers_list_debug():
    try:
        staff = (session.get("staff_name") or "").lower()
        resellers = fb_get("resellers") or {}
        return jsonify({"ok": True, "staff": staff, "resellers_raw_count": len(resellers), "sample_keys": list(resellers.keys())[:5] if resellers else []})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
    except Exception as e:

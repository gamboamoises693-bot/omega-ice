...
async function loadCustomers(){
  try{
    ...
    document.getElementById('tbody').innerHTML=filtered.map(r=>{
      ...
      return `<tr><td><b>${r.store_name}</b><br><small style="color:#666">${r.status||'active'}</small></td><td>${r.phone}<br><small style="color:${r.password_hash?'green':'red'}">${r.password_hash?'Has password':'No password'}</small></td><td>₱${r.credit_balance||0}</td><td><button class="btn" style="background:#22c55e;color:#fff" onclick="openEdit('${r.id}','${safeStore}','${r.phone}')">Edit</button></td></tr>`;
    }).join('');
  }catch(e){
...
function openEdit(id,store,phone){editingId=id;document.getElementById('editStore').textContent=store;document.getElementById('editPhone').value=phone;document.getElementById('editPassword').value='';document.getElementById('editCard').style.display='block';window.scrollTo({top:document.getElementById('editCard').offsetTop,behavior:'smooth'});}

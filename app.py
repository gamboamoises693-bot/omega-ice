    ...
    if(hasProfit){
      ...
      months.forEach(m => {
        ...
        bars += `<div class="bar-col" style="cursor:pointer" title="${escapeHtmlCT(m.label)} ${data.year} - Binili: ${escapeHtmlCT(peso(m.total))}, Kita: ${escapeHtmlCT(peso(profitVal))} - tap para tingnan ang mga order" onclick="showMonthBreakdown(${m.month})">
          <div class="bar-stack">
            ${m.total > 0 ? `<div class="stack-label cost-lbl" style="bottom:calc(${costPct}% + 3px)">${escapeHtmlCT(pesoShort(m.total))}</div>` : ''}
            ${profitVal !== 0 ? `<div class="stack-label profit-lbl${profitNeg?' neg':''}" style="bottom:calc(${profitPct}% + 13px)">${profitNeg?'-':''}${escapeHtmlCT(pesoShort(Math.abs(profitVal)))}</div>` : ''}

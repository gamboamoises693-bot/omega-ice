...
# Sept 21, round-number revision): reseller's real all-time margin is
# 27.1%, and the shop's minimum order is 50Kg (= P500 = 500 points
# GUARANTEED on every single order, even the very first one) - every
# tier must sit well above that so a reward actually takes repeat
# business, not just one order. ISESMO asked for a clean 1,000-point
# floor on the entry-level reward instead of the previous 700, so this
# table uses a single, consistent 100x-price multiplier across every
# tier (points_required = price x 100) - both round numbers that are
# easy to explain to resellers, AND a much higher multiplier than any
# previous revision, which only pushes the breakeven-margin requirement
# further down (safer) - see get_reward_breakeven_margin. ISESMO can
# retune these anytime from /admin/rewards without a code change - the
# margin health check on that page always shows the live breakeven math
# for whatever values are actually saved.

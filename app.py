...
import os, sqlite3, json, requests, time, base64, threading, smtplib, socket
...
<a href="/customer/{{ reseller_id }}/trend" style="padding:8px 12px;border-radius:20px;border:1px solid #cde;background:#eef4fb;color:#00609C;font-size:11px;text-decoration:none;font-weight:600">📈 Sales Trend</a>
</div>
...
<div class="card">
  ...
  <div class="price-hint">Ito yung presyo na ibinebenta mo sa customers mo per kilo ng yelo - gagamitin para awtomatikong makuha ang tinatayang kita mo.</div>
...
<div class="card">
  <div class="section-title">Buwanang Trend (Binili)</div>
...
async function saveRetailPrice(){
  ...
  try{
    const res = await fetch(`/api/customer/${resellerId}/retail_price`, {
      method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({retail_price_per_kg: Number(val)}),
    });
    const data = await res.json();
    if(!data.ok){ statusEl.textContent = data.error || 'Error'; statusEl.className = 'price-status err'; return; }
    statusEl.textContent = 'Na-save! Na-update na ang profit analysis sa baba.';
    ...
    if(months.some(m => m.profit !== null && m.profit !== undefined)){
      ...
      tbody.innerHTML = months.map(m => {
        ...
        return `<tr>
          ...
          <td class="${profitClass}">${escapeHtmlCT(peso(m.profit||0))}</td>
        </tr>`;
      }).join('');
    }else{
      profitTableCard.style.display = 'none';
...
async function showMonthBreakdown(month){
  ...
  try{
    ...
    html += `<table class="profit-table"><thead><tr><th>Petsa</th><th>Qty x Kg</th><th>Halaga</th><th>Status</th>${IS_STAFF ? '<th></th>' : ''}</tr></thead><tbody>`;
    ...
    html += '</tbody></table>';
    if(IS_STAFF){
      html += '<div class="price-hint" style="margin-top:8px">⚠️ Staff lang: ang pag-delete dito ay permanent at hindi na maibabalik. Gamitin lang kung sigurado kang duplicate/maling entry.</div>';
    }
    area.innerHTML = html;
...
async function deleteBreakdownOrder(saleId){
  ...
  try{
    const res = await fetch(`/api/sale/${saleId}`, {method: 'DELETE'});
    const data = await res.json();
...
def api_customer_yearly_trend(reseller_id):
    ...
    binenta nya ng 15"):
      ...
      - retail_value = kg * their own saved retail_price_per_kg (the
        "benta" side - what THEY charge their own end customers).
      - profit = retail_value - total (the estimated "kita").
    profit/retail_value are null until the reseller sets a retail price
    (see /api/customer/<reseller_id>/retail_price) - we never guess a
    number for them.
    """
    if session.get("customer_id") and session.get("customer_id") != reseller_id:
        return jsonify({"ok": False, "error": "Not allowed"}), 403
    ...
    try:
        ...
        retail_price = reseller.get("retail_price_per_kg")
        try:
        ...
        for val in sales.values():
            ...
            if 0 <= month_idx < 12:
                ...
                qty = int(val.get("quantity", 0) or 0)
                month_kg[month_idx] += qty * kg_val(val.get("kg_size", "1Kg"))
        ...
        for i in range(12):
            ...
            if retail_price is not None:
                retail_value = round(kg * retail_price, 2)
        ...
        if retail_price is not None:
            year_retail_value = round(year_kg * retail_price, 2)
...
def api_customer_set_retail_price(reseller_id):
    ...
    try:
        ...
        if not fb_get(f"resellers/{reseller_id}"):
            return jsonify({"ok": False, "error": "Reseller not found"}), 404
        fb_patch(f"resellers/{reseller_id}", {"retail_price_per_kg": price})
        return jsonify({"ok": True, "retail_price_per_kg": price})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
function renderTable(){
  ...
  tbody.innerHTML = rows.map(r => `
    <tr class="clickable" onclick="openDetail('${r.id}')">
      <td><div class="reseller-name">${escapeHtmlR(r.store_name)}</div><div class="reseller-phone">${escapeHtmlR(r.phone || '')}</div></td>

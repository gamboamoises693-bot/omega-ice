...
# Sept 21, safety revision): reseller's real all-time margin was 27.1%.
# These thresholds use a 6x-price multiplier (raised from the original
# 4x), which lowers the breakeven margin needed to ~14.3% - a ~12.8
# percentage-point safety buffer above break-even on every redemption
# (up from ~7.1pp at 4x), giving much more room to absorb normal margin
# swings (electricity/consumables cost spikes) before any reward
# redemption risks a loss. ISESMO can retune these anytime from
# /admin/rewards without a code change - the margin health check on
# that page always shows the live breakeven math for whatever values
# are actually saved.
DEFAULT_REWARD_CATALOG = {
    "reward_1kg": {"label": "Libreng 1Kg Ice", "kg_size": "1Kg", "quantity": 1, "points_required": 60},
    "reward_5kg": {"label": "Libreng 5Kg Ice", "kg_size": "5Kg", "quantity": 1, "points_required": 300},
    "reward_10kg": {"label": "Libreng 10Kg Ice", "kg_size": "10Kg", "quantity": 1, "points_required": 600},
    "reward_25kg": {"label": "Libreng 25Kg Ice", "kg_size": "25Kg", "quantity": 1, "points_required": 1500},
}
...
def get_points_expiry_days():
    ...
    /admin/rewards without a redeploy; falls back to a 90-day default
    (~3 months - long enough to not punish a normal slow month, short
    enough that a truly inactive/abandoned account doesn't sit as an
    open-ended rewards liability forever)."""
    try:
...
def get_current_margin_pct():
    """
    All-time real profit margin %, computed with the EXACT SAME
    functions the Home Dashboard's "ALL TIME" + "Include Fixed Asset:
    ON" view already uses (imported from modules.home_dashboard rather
    than re-derived here) - so this safety check always agrees with
    the number ISESMO already watches on /home, instead of drifting out
    of sync with a second copy of the same math. Returns None if there
    are no sales yet to compute a margin from.
    """
    try:
        ...
        total_exp = operating_exp + fixed_amt
        if total_sales <= 0:
            return None
...
<div class="card" style="background:linear-gradient(135deg,#00609C,#0f2942);color:#fff">
  ...
  <div style="font-size:10px;opacity:.8;margin-top:6px">Kumikita ng points sa bawat online order na na-DELIVER (hindi kasama ang manual/walk-in sale)</div>
...
function renderRewardsList(){
  ...
  el.innerHTML = _rewardsCache.map(r => `
    ...
    try:
        ...
        if balance > 0 and last_earned:
            try:
                ...
                expires_at = None
        return jsonify({"ok": True, "balance": balance, "rewards": rewards, "expires_at": expires_at})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
<body>
<div class="topbar"><h1>Live Customer Orders</h1><div class="pill-group"><a href="/cashier" class="nav-pill">Sales</a> <a href="/customers" class="nav-pill">Customers</a> <a href="/credit" class="nav-pill">💳 Utang</a></div></div>
...
const list=document.getElementById('ordersList');
  ...
  list.innerHTML=orders.map(o=>{
    ...
    return `<div class="order-card" data-order-id="${o.id}"><div style="display:flex;justify-content:space-between"><span style="font-weight:600">${o.reseller_name}${rewardBadge}</span><span style="font-size:10px;background:${statusColor};padding:4px 8px;border-radius:12px">${o.order_status}</span></div><div style="font-size:12px;color:#555;margin-top:4px">${o.quantity}x ${o.kg_size} • ${priceLabel} • ${o.sales_date}</div><div style="margin-top:8px"><button class="btn" ${btnDisabled} style="${btnStyle()}" onclick="updateStatus('${o.id}','Pending')">Accept</button><button class="btn" ${btnDisabled} style="${btnStyle()}" onclick="updateStatus('${o.id}','Preparing')">Preparing</button><button class="btn" ${btnDisabled} style="${btnStyle()}" onclick="updateStatus('${o.id}','Out for Delivery')">Out</button><button class="btn" ${btnDisabled} style="background:#22c55e;color:#fff;${btnStyle()}" onclick="updateStatus('${o.id}','Delivered')">Done</button><button class="btn" style="background:#fff;color:#ef4444;border-color:#fca5a5" onclick="deleteOrder('${o.id}')">🗑️ Delete</button></div></div>`;
  }).join('');
}
...
<body>
<div class="topbar"><h1>🎁 Rewards Catalog (ISESMO Only)</h1><a href="/cashier" class="nav-pill">← Sales</a></div>
...
async function loadMarginHealth(){
  ...
  el.innerHTML = 'Loading...';
  try{
    const res = await fetch('/api/admin/margin_health');
    const data = await res.json();
    if(!data.ok){ el.innerHTML = `<div style="color:red">${escapeHtmlR(data.error||'Error')}</div>`; return; }
...
def api_admin_get_loyalty_settings():
    try:
        return jsonify({"ok": True, "inactivity_days": get_points_expiry_days()})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def api_admin_save_loyalty_settings():
    """Sets how many days a reseller can go with no delivered online
    order before their points expire (see check_and_expire_points).
    Rejects anything under 1 day so a fat-fingered value can't wipe
    every reseller's points on their very next page load."""
    try:
        data = request.json or {}
        ...
        return jsonify({"ok": True, "inactivity_days": days})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def api_admin_margin_health():
    ...
    """
    try:
        current_margin = get_current_margin_pct()
        catalog = get_reward_catalog()
        rows = []

...
async function loadStuck(){
  ...
  try{
    ...
    const orders = data.orders || [];
    if(!orders.length){ wrap.innerHTML = '<div class="empty">✅ Walang stuck orders - malinis!</div>'; return; }
    wrap.innerHTML = orders.map(o => {
      ...
      return `<div class="order-card ${isOrphan?'orphan':''}">
        ...
        <button class="del-btn" onclick="purgeOrder('${o.id}', this)">🗑️ Permanent Delete</button>
      </div>`;
    }).join('');
  }catch(e){
...
async function purgeOrder(id, btn){

...
<div class="total-card"><div class="amt" id="yearTotal">₱0</div><div class="lbl" id="yearTotalLbl">TOTAL SALES FOR SELECTED YEAR</div></div>
...
async function loadTrend(){
  ...
  try{
    ...
    document.getElementById('yearTotalLbl').textContent = `TOTAL SALES - ${data.year}`;
...
def api_customer_yearly_trend(reseller_id):
    ...
    try:
        ...
        months = [
            {"month": i + 1, "label": MONTH_LABELS_CUST[i], "total": round(month_totals[i], 2)}
            for i in range(12)
        ]

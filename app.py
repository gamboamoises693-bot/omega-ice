...
def trigger_route_delivery_alert(delivered_reseller_id):
    """Route-mate delivery alert (boss's request, Sept 26): "kaya ba
    ilagay ng route customer, tapos pag may na-deliver sa route na yun
    mag-notify sa mga same route" - but WITHOUT naming which store just
    got delivered (boss's exact words: "di sasabihin anong store name,
    sasabihin lang na may delivery area at pwede na din sila mag place
    order para maisabay").
    ...
    Design (per boss's answers, Sept 26):
      ...
      - 30-minute cooldown per reseller so back-to-back deliveries on
        the same route don't spam the same customer's phone repeatedly.
    ...
    Called only from the Delivered branch of api_update_order_status().
    Wrapped in its own try/except (like every other post-delivery side
    effect in that function - points, referral bonus) so a hiccup here
    NEVER blocks the actual delivery from being recorded."""
    try:
        reseller = fb_get(f"resellers/{delivered_reseller_id}") or {}
        ...
        message = "May delivery ngayon sa lugar niyo! Pwede ka nang mag-order para maisabay."
        notified = 0
        for rid, rdata in (resellers or {}).items():
            if not rdata or rid == delivered_reseller_id:
                continue
            ...
            existing_alert = rdata.get("route_alert") or {}
            last_alert_at = existing_alert.get("triggered_at") if isinstance(existing_alert, dict) else None
            if last_alert_at:
                try:
                    last_dt = datetime.strptime(last_alert_at, "%Y-%m-%d %H:%M:%S")
                    if (now_ts - last_dt).total_seconds() < 1800:
                        continue  # cooldown - alerted this same customer recently
            ...
            try:
                send_push_to_reseller(
                    rid,
                    title="🚚 May Delivery sa Lugar Niyo!",
                    body=message,
        ...
        if notified:
            fb_post("route_delivery_alerts", {
                "route": route,
                "delivered_reseller_id": delivered_reseller_id,
                "notified_count": notified,
                "timestamp": now_ts.strftime("%Y-%m-%d %H:%M:%S"),
            })
    except Exception as e:
        print(f"trigger_route_delivery_alert failed (non-fatal): {e}")
...
<div id="routeAlertBanner" style="display:none;background:#ecfdf5;border:1px solid #a7f3d0;border-radius:10px;padding:10px;margin-bottom:10px;font-size:12px;color:#065f46">
  ...
  <a href="/customer/{{ reseller_id }}/order" style="display:inline-block;margin-top:8px;padding:7px 14px;border-radius:20px;background:#059669;color:#fff;text-decoration:none;font-size:11px;font-weight:700">🧊 Mag-order Ngayon</a>
</div>
...
def api_customer_orders(reseller_id):
    ...
    try:
        ...
        route_alert_out = {"active": bool(route_alert.get("active")), "message": route_alert.get("message") or ""} if isinstance(route_alert, dict) else {"active": False, "message": ""}
...
def api_update_order_status(order_id):
    ...
    if new_status == "Delivered":
        ...
        # ROUTE DELIVERY ALERT (boss's request, Sept 26): nudge other
        # customers tagged to the same delivery route, without naming
        # which store just got delivered. See trigger_route_delivery_alert()
        # docstring for the full design - wrapped in its own try/except
        # there so this never blocks the delivery itself.
        if existing.get("reseller_id"):
            trigger_route_delivery_alert(existing.get("reseller_id"))
...
<table><thead><tr><th>Store</th><th>Phone / Login</th><th>Route</th><th>Balance</th><th>Action</th></tr></thead>
<tbody id="tbody"><tr><td colspan=5 style="text-align:center;padding:20px;color:#999">Loading...</td></tr></tbody>
</table>
</div>
</div>
...
async function loadCustomers(){
  try{
    ...
    if(!rows.length){
      document.getElementById('tbody').innerHTML='<tr><td colspan=5 style="text-align:center;padding:20px;color:#888">No customers yet. '+(data.error||'')+'</td></tr>';
      return;
    }
...
def api_customers_list():
    try:
        ...
        for key,val in resellers.items():
            if not val: continue
            out.append({"id":key,"store_name":val.get("store_name") or val.get("name") or "No Name","phone":val.get("phone") or val.get("contact",""),"credit_balance":val.get("credit_balance",0),"password_hash":"yes" if val.get("password_hash") else "","status":val.get("status","active"),"referred_by_name":val.get("referred_by_name") or "","route":val.get("route") or ""})
...
def api_set_reseller_route(reseller_id):
    ...
    trigger_route_delivery_alert() to find "same route" customers when
    an order in that route gets delivered - no geolocation, just
    whatever ISESMO types in. ISESMO-only, same gate as every other

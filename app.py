...
async function loadOrders(){
  try{
    ...
    const priority = {"New Order":0, "Pending":1, "Preparing":2, "Out for Delivery":3, "Delivered":4, "Declined":5, "Cancelled":6};
...
def api_customer_orders(reseller_id):
    ...
    try:
        ...
        for key,val in sales.items():
            ...
            orders.append({"id": key, "sales_date": val.get("sales_date"), "quantity": qty, "kg_size": kg_size, "total_sales": peso, "mode": val.get("mode"), "payment": val.get("payment"), "order_status": status, "created_at": val.get("created_at"), "rating": val.get("rating"), "feedback": val.get("feedback"), "decline_reason": val.get("decline_reason") or ""})
        def status_priority_c(s):
            order = (s.get("order_status") or "Pending")
            priorities = {"New Order": 0, "Pending": 1, "Preparing": 2, "Out for Delivery": 3, "Delivered": 4, "Declined": 5, "Cancelled": 6}

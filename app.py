...
function renderRewardsList(){
  ...
  // Cooldown note goes ABOVE the reward rows so it explains up front
  // why every "Redeem" button might be greyed out, even for a reward
  // the reseller clearly has enough points for.
  let html = '';
  if(_cooldownDaysLeft > 0){
    ...
    try:
        ...
        for key, val in catalog.items():
            ...
            rewards.append({
                ...
                "can_redeem": balance >= points_required and cooldown_days_left <= 0,
            })

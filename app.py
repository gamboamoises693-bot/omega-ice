...
# Sept 21, final revision): reseller's real all-time margin is 27.1%,
# and the shop's minimum order is 50Kg (= P500 = 500 points GUARANTEED
# on every single order, even the very first one) - every tier must sit
# well above that so a reward actually takes repeat business, not just
# one order. ISESMO bumped the entry-level reward from 1,000 to 2,000
# points (~4 orders instead of ~2) for extra breathing room, keeping
# 5,000/10,000/25,000 for the rest - not a single uniform multiplier
# anymore, but still a clean, easy-to-explain progression (4/10/20/50
# orders) and each tier's own multiplier only pushes its individual
# breakeven-margin requirement further down (safer) - see
# get_reward_breakeven_margin. ISESMO can retune these anytime from
...
DEFAULT_REWARD_CATALOG = {
    "reward_1kg": {"label": "Libreng 1Kg Ice", "kg_size": "1Kg", "quantity": 1, "points_required": 2000},

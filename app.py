...
<div class="card" style="border:2px solid #f59e0b;background:#fffbeb">
  ...
  <div style="font-size:10px;color:#92400e;background:#fef3c7;padding:8px;border-radius:8px;margin-bottom:10px">🔒 Safe dito boss! Dito lang lilitaw OTP, hindi sa Sales.</div>
...
<div class="card"><div style="overflow-x:auto"><table><thead><tr><th>Store</th><th>Phone / Login</th><th>OTP / Status</th><th>Action</th></tr></thead><tbody id="tbody"><tr><td colspan=4 style="text-align:center;padding:20px;color:#999">Loading customers... Please wait<br><small>If stuck, check if logged in as ISESMO</small></td></tr></tbody></table></div></div>
...
<div class="qr-modal" id="qrModal" onclick="if(event.target===this)closeQR()">
  <div class="qr-box">
    ...
    <p style="font-size:9px;color:#999;margin-top:8px">Pag na-scan ng reseller, auto-login na!</p>
  </div>
</div>
<script>
let editingId=null;
...
async function loadCustomers(){
  try{
    ...
    let data;
    try{data=JSON.parse(text);}catch(e){document.getElementById('customerCount').textContent='Not JSON: '+text.substring(0,100); document.getElementById('tbody').innerHTML='<tr><td colspan=4 style="color:red">Login expired? Please login again as ISESMO<br><a href="/login">Go to Login</a><br><small>'+text.substring(0,200)+'</small></td></tr>'; return;}
    const rows=data.resellers||[];
    const otps=data.otps||{};
    document.getElementById('customerCount').textContent=rows.length+' customers found'+(data.error?' - Error: '+data.error:'');
    if(!rows.length){document.getElementById('tbody').innerHTML='<tr><td colspan=4 style="text-align:center;padding:20px;color:#888">No customers yet.<br>Records safe in Firebase</td></tr>'; return;}
...
async function loadOTPs(){
  try{
    ...
    const text=await res.text();
    let data;
    try{data=JSON.parse(text);}catch(e){document.getElementById('otpList').innerHTML='<div style="color:#c0392b;font-size:12px;text-align:center">Login expired? Please login again<br><a href="/login">Login</a><br><small>'+text.substring(0,200)+'</small></div>'; return;}
    ...
    if(!otps.length){
      document.getElementById('otpList').innerHTML='<div style="text-align:center;padding:20px;color:#999;font-size:12px">No OTP requests yet...</div>';
      return;
    }
    document.getElementById('otpList').innerHTML=otps.map(o=>{
      ...
      return `<div style="background:${bg};border:1px solid ${border};border-radius:10px;padding:10px;font-size:12px"><div style="display:flex;justify-content:space-between;margin-bottom:6px"><b>📱 ${o.phone}</b><span style="font-size:9px;padding:2px 6px;border-radius:8px;background:#fff;border:1px solid ${border}">${isUsed} • ${isExpired}</span></div><div style="display:flex;gap:8px;margin:6px 0"><div style="font-size:18px;font-weight:800;letter-spacing:3px;background:#fff;padding:6px 10px;border-radius:8px;border:2px dashed #f59e0b">${o.otp}</div><button class="btn btn-otp" onclick="navigator.clipboard.writeText('${o.otp}').then(()=>alert('Copied'))">📋 Copy</button></div><div style="font-size:10px;color:#666;display:flex;justify-content:space-between"><span>⏰ ${o.created_at}</span><span>Exp: ${o.expires_at}</span></div>${o.reseller_name?`<div style="font-size:10px;color:#00609C;margin-top:4px">👤 ${o.reseller_name}</div>`:''}</div>`;
    }).join('');
  }catch(e){

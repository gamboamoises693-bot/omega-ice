...
.card .credit-date{font-size:10px;color:#999;margin-top:2px}
...
<div class="topbar">
  ...
  <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
    ...
    <a href="/credit/history" class="nav-pill" style="background:#00609C;color:#fff;border-color:#00609C">🧾 Sino Nag-collect?</a>
    <div class="menu-wrap">
...
function formatCreditDate(iso){
  if(!iso) return '';
  // Handles both a full "2026-09-25T10:30:00" timestamp and a plain
  // "2026-09-25" date string (older sales may only have the latter).
...
function renderList(){
  ...
  wrap.innerHTML = rows.map(r => {
    const dateLabel = formatCreditDate(r.last_credit_at);
    return `
    <div class="card">
      <div>
        <div class="name">${escapeHtmlU(r.store_name)}</div>
        <div class="bal">${peso(r.credit_balance)} outstanding</div>
        ${dateLabel ? `<div class="credit-date">Huling utang: ${dateLabel}</div>` : ''}
      </div>
...
.empty{color:#888;text-align:center;padding:30px 10px;font-size:13px}
.del-btn{background:none;border:none;color:#c0392b;font-size:16px;padding:2px 4px;cursor:pointer}
.restore-btn{background:none;border:none;color:#00609C;font-size:16px;padding:2px 4px;cursor:pointer}
</style></head>
<body>
<div class="topbar">
  ...
  wrap.innerHTML = rows.map(r => `
    <div class="log-row">
      ...
      <div style="display:flex;align-items:center;gap:8px">
        ...
        ${IS_ISESMO ? `
        <button class="restore-btn" data-id="${escapeHtmlH(r.id)}" title="Restore Utang - ibalik ang amount na ito pabalik sa utang ng customer" onclick="restorePaymentByIdx(this)">↩️</button>
        <button class="del-btn" data-id="${escapeHtmlH(r.id)}" title="Permanent Delete - tanggalin sa log lang, hindi babalik sa utang" onclick="deletePaymentByIdx(this)">🗑️</button>
        ` : ''}
      </div>
    </div>
  `).join('');
...
function restorePaymentByIdx(btn){
  const id = btn.getAttribute('data-id');
  const r = allPayments.find(x => String(x.id) === id);
  if(!r) return;
  removePayment(r.id, r.store_name, r.amount, true);
}
function deletePaymentByIdx(btn){
  const id = btn.getAttribute('data-id');
  const r = allPayments.find(x => String(x.id) === id);
  if(!r) return;
  removePayment(r.id, r.store_name, r.amount, false);
}

// restore=true  -> "Restore Utang": bawiin ang payment, ibalik ang amount
//                  pabalik sa credit_balance ng customer (parang hindi
//                  nangyari - para sa mali/test na entry).
// restore=false -> "Permanent Delete": tanggalin lang sa log, HINDI
//                  babalik sa utang - dahil totoong nabayaran naman ito,
//                  linis-log lang (hal. duplicate entry, typo sa note).
async function removePayment(id, storeName, amount, restore){
  const msg = restore
    ? `Ibalik ang ${peso2(amount)} pabalik sa utang ni ${storeName}?\\n\\nGamitin ito kung MALI o TEST lang ang entry na ito - parang hindi ito nangyari, at babalik ito sa "Utang" list niya.`
    : `Permanenteng tanggalin ang log entry na ${peso2(amount)} para kay ${storeName}?\\n\\nHINDI ito babalik sa utang niya - nananatiling bayad na ang customer. Ito lang, tatanggalin sa Collection History log (hal. duplicate entry).`;
  if(!confirm(msg)) return;
  try{
    const res = await fetch(`/api/credit/payment/${id}?restore=${restore}`, {method:'DELETE'});
    const data = await res.json();
    if(data.ok){
      loadHistory();
    } else {
...
def api_credit_outstanding():
    try:
        ...
        # Build a per-reseller "when was utang last incurred" lookup by
        # scanning daily_sales for Credit-mode sales. credit_balance itself
        # is just a running total (it can be the sum of many sales over
        # weeks), so there's no single date stored on the reseller record -
        # the actual dated events live in daily_sales, keyed by reseller_id.
        # We take the LATEST Credit sale per reseller (most relevant to
        # "kailan na-utang"), not the oldest, since that's what a collector
        # actually wants to know when following up.
        last_credit_at = {}
        sales = fb_get("daily_sales") or {}
        for s_val in sales.values():
            if not s_val or s_val.get("payment") != "Credit":
                continue
            rid = s_val.get("reseller_id")
            if not rid:
                continue
            # created_at is a full timestamp (more precise than sales_date
            # alone), fall back to sales_date if created_at is missing.
            when = s_val.get("created_at") or s_val.get("sales_date") or ""
            if not when:
                continue
            if rid not in last_credit_at or when > last_credit_at[rid]:
                last_credit_at[rid] = when
        ...
        for key, val in resellers.items():
            ...
            if bal > 0:
                rows.append({
                    "id": key,
                    "store_name": val.get("store_name") or "(unnamed)",
                    "credit_balance": bal,
                    "last_credit_at": last_credit_at.get(key, ""),
                })
                total += bal
...
def api_credit_delete_payment(payment_id):
    """Remove a payment entry from the Collection History log - restricted
    to Isesmo since this touches money already recorded as collected,
    which the rest of the staff shouldn't be able to quietly undo.

    Two distinct modes, chosen via ?restore=true|false (defaults to true
    for backward compatibility with any old client still calling this
    without the param):

      - restore=true  ("Restore Utang"): the payment never actually
        happened (test entry, wrong customer, etc.) - the amount is
        added BACK onto the reseller's outstanding credit_balance, and
        the log entry is removed. This is the ORIGINAL/only behavior
        this endpoint had before, kept as the default.

      - restore=false ("Permanent Delete"): the payment DID happen and
        stays honored (the customer really paid, credit_balance is left
        untouched) - only the log entry itself is removed, e.g. to clean
        up a duplicate log line or a note typo without re-opening the
        customer's utang.
    """
    try:
        restore = (request.args.get("restore", "true").lower() != "false")
        ...
        if restore and reseller_id:
            reseller = fb_get(f"resellers/{reseller_id}") or {}
            current_balance = float(reseller.get("credit_balance") or 0)
            # Give the amount back to the outstanding balance - deleting a
            # payment (in "restore" mode) means it never happened, so the
            # utang is restored.

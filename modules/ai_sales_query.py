...
Question: {question}"""
    ...
    # TEMP DEBUG (remove once the URL issue is confirmed fixed): this goes to
    # Render's Logs tab only - never to the browser/chat UI - so we can read
    # the exact server-side string with zero risk of a phone browser (e.g.
    # Chrome auto-translate) rewriting it before we see it.
    print(f"[ai-sales DEBUG] len={len(api_url)} api_url={api_url!r}", flush=True)
    payload = {
    ...
    try:
        ...
        print(f"[ai-sales DEBUG] request failed, response.url={getattr(e.response, 'url', None)!r}", flush=True)

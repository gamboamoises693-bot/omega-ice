...
if(!SPEECH_SYNTH_SUPPORTED){
  ...
  voiceReplyBtn.title = 'Hindi supported ang voice output sa browser na ito - gamitin ang Chrome.';
...
function speakReply(text){
  ...
  try {
    window.speechSynthesis.cancel(); // wag mag-overlap sa sunod-sunod na reply
    // Pause hands-free listening while the AI talks, kung naka-ON ito -
    // iiwas sa feedback loop (mic naririnig yung sariling boses ng AI).
    ...
    const filVoice = voices.find(v => v.lang.toLowerCase().includes('fil')) || voices.find(v => v.lang.toLowerCase().includes('en'));
    if(filVoice) utter.voice = filVoice;

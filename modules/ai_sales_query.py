...
Question: {question}"""
    ...
    # Retry ONLY on 503/429 (Gemini "model overloaded" / rate-limit - both
    # transient, both common on a free-tier key) - one retry, short backoff.
    # Deliberately NOT retrying on a timeout/connection error: that attempt
    # already burned its own timeout budget, and gunicorn kills this whole
    # worker if ONE request handling takes longer than ~30s (that's the
    # WORKER TIMEOUT bug from Sept 25, 2026 - see _manila_today's history).
    # Budget here is capped well under that: 2 attempts x 12s timeout + a
    # 1.5s backoff = 25.5s worst case, so a hung request still fails fast
    # enough to never trip the worker timeout again.
    last_exc = None
    resp = None
    for attempt in (1, 2):
        try:

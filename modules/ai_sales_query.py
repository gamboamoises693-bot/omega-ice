...
SETUP REQUIRED:
    1. Get an Anthropic (Claude) API key: https://console.anthropic.com/
       (Sept 26 2026: switched from Gemini to Claude at ISESMO's request -
       see _ask_claude_for_date_range below. Gemini is no longer used
...
def api_ai_sales_ask():
    ...
    # atbp). Zero latency, zero Claude API usage, immune sa rate limits/
    # overload. Kung hindi ma-match (freeform/ambiguous question), saka pa
    # lang tatawagin si Claude.
    parsed = _try_local_date_extraction(question, today)
    err = None
    if parsed is None:

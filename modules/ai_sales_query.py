...
Question: {question}"""
    ...
    try:
        ...
        # TEMP DEBUG (remove once the URL issue is confirmed fixed): echoes
        # exactly what this server process computed BEFORE calling requests,
        # so we can tell whether the URL was already wrong here, or only
        # became wrong somewhere inside requests/urllib3/network.
        masked = (api_key[:6] + "..." + api_key[-4:]) if len(api_key) > 12 else "(short key)"
        return None, (
            f"Hindi ma-reach ang AI service: {e} "
            f"[debug: GEMINI_MODEL={GEMINI_MODEL!r} | api_url_built={api_url} | key={masked}]"
        )

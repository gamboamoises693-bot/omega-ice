...
def _ask_claude_for_date_range(question, today_str):
    ...
    if not api_key:
        return None, "ANTHROPIC_API_KEY is not set in your environment variables (Render > Environment)."
...
Question: {question}"""
    ...
    if last_exc is not None or resp is None or not resp.ok:
        ...
        if status == 429:
            return None, "The AI rate limit has been reached. Please wait a bit and try again in a minute."
        if status == 529:
            return None, "The AI service is very busy right now (this is on Claude's API side, not our app). Please try again in a few seconds."
        if status == 401:
            return None, "The ANTHROPIC_API_KEY is invalid or expired - please check it in Render > Environment."
        detail = str(e) if e else f"{status} Server Error for url: {api_url}"
        return None, f"Could not reach the AI service: {detail}"
    ...
    if not parsed:
        return None, "The AI couldn't understand your question - please try again with a clearer date (e.g. 'sales last May')."
    return parsed, None
...
def _format_sales_reply(label, summary):
    ...
    if summary["count"] == 0:
        lines.append("")
        lines.append("No sales recorded for this period.")
        return "\n".join(lines)
...
_LOYALTY_KEYWORDS = re.compile(
    r"\b(points?|puntos|loyalty(?:\s*program)?|rewards?|redeem(?:ed|ing)?|"
    r"redemption|reward points|points balance)\b", re.I
)
_MACHINE_KEYWORDS = re.compile(
    r"\b(machines?|makina|equipment|harvest|preventive maintenance|"
    r"maintenance due|filter change|filter replacement|pm schedule|"
    r"servicing)\b", re.I
)
_STAFF_KEYWORDS = re.compile(
    r"\b(staff|empleyado|employees?|tauhan|personnel|team members?)\b", re.I
)
_RESELLER_INFO_KEYWORDS = re.compile(
    r"\b(credit balance|outstanding balance|utang|owes?|presyo per ?kg|"
    r"price per ?kg|contact number|phone number|listahan ng resellers?|"
    r"list of resellers?|mga reseller|resellers ko|my resellers|"
    r"reseller info(?:rmation)?|reseller (?:profile|details)|"
    r"customer info(?:rmation)?|last order|huling order|huling bili|"
    r"huling benta|last purchase|last transaction|profit|kinita|kumikita|"
    r"kikitain|margin|earnings?|earned|ilang beses|how many times|"
    r"order count|bilang ng order|how many orders)\b", re.I
)
...
_PROFIT_INTENT = re.compile(
    r"\b(profit|kinita|kumikita|kikitain|margin|earnings?|earned)\b", re.I
)
...
_EXPENSE_KEYWORDS = re.compile(
    r"\b(expenses?|spending|gastos|electric\w*|utilit(?:y|ies)|kuryente|"
    r"koryente|ilaw|consumables?|fuel|gasolina|diesel|maintenance|plastic|"
    r"packaging|bags?|bills?)\b", re.I
)
...
_PLASTIC_PURCHASE_INTENT = re.compile(
    r"\b(nabili|binili|bumili|bought|purchased|purchase|buy|buying)\b", re.I
)
...
def _handle_resellers_question(question, today):
    ...
    if not resellers:
        return "No resellers recorded in the system yet."
    ...
    if (wants_last_order or wants_order_count or wants_profit) and not match:
        return ("I couldn't detect which reseller/customer you meant. "
                "Please type the store's full name "
                '(e.g. "profit for Nena\'s Ice Store this month").')
    ...
    if match:
        ...
        if wants_profit:
            ...
            if not price:
                return f"{name} doesn't have a retail price set yet, so I can't calculate their profit."
            date_from, date_to, is_all_time, label = _resolve_question_range(question, today)
            orders = _get_reseller_orders_in_range(key, name, date_from, date_to, is_all_time)
            if not orders:
                return f"No orders recorded for {name} in this period."
            ...
            return "\n".join([
                ...
                f"🏷️ Retail Price/Kg (current): {_peso(price)}",
                f"📦 Total Kilos Purchased: {total_kg:,.1f} kg",
                f"💳 Paid to Omega: {_peso(total_paid)}",
                f"💰 Estimated Earnings: {_peso(est_profit)}",
                f"📊 Average Profit per Order: {_peso(avg_profit)} ({len(orders)} order/s)",
                "",
                "⚠️ This is only an estimate based on their CURRENT retail price - "
                "the actual figure may differ if their price changed over time.",
            ])

        name_line = name
        contact = match.get("contact") or match.get("phone") or "Not recorded"
        ...
        if wants_last_order:
            ...
            if last:
                ...
                lines.append(f"🕒 Last Order: {last['_date']}")
                lines.append(f"   {qty} x {kg_size} — {_peso(peso)} ({pay})")
            else:
                lines.append("🕒 Last Order: No orders recorded for this customer yet.")
    ...
    if re.search(r"\bcredit|utang|balance\b", q):
        ...
        lines = [
            f"💳 Have Credit Balance: {len(with_credit)} of {len(resellers)} reseller(s)",
            f"💰 Total Credit Outstanding: {_peso(total_credit)}",
        ]
        if with_credit:
            lines.append("")
            lines.append("Biggest Outstanding Balances:")
    ...
    lines.append("")
    lines.append('Tip: ask for a store by name (e.g. "credit balance for [store name]") for details.')
    return "\n".join(lines)
...
def _handle_loyalty_question(question):
    ...
    if rmatch and _REDEMPTION_COUNT_INTENT.search(q) and re.search(r"\breward|redeem\b", q):
        ...
        lines = [f"🎁 {name} — Reward Redemptions", "", f"🔢 Redemption Count: {len(redemptions)}"]
        if redemptions:
            lines.append("")
            lines.append("Most Recent Redemptions:")
    ...
    if not rmatch and re.search(r"\breward|catalog|redeem\b", q) and not re.search(r"\btop|pinakamaraming|highest|leader\b", q):
        ...
        if not catalog:
            return "No reward catalog has been set up yet."
    ...
    if rmatch:
        ...
        last_earned = entry.get("last_earned_at") or "None yet"
        last_redeem = entry.get("last_redemption_at") or "None yet"
        name = rmatch.get("store_name") or "Unknown"
        return "\n".join([
    ...
    if not ranked:
        return "No reseller has any loyalty points right now."

    total_points = sum(b for _, b in ranked)
    lines = [f"🏆 Top Loyalty Points Holders", "", f"⭐ Total Points (All Resellers): {int(total_points):,}", ""]
...
def _handle_staff_question(question):
    ...
    if not records:
        return "No staff records in the system yet."
...
def _handle_machines_question(question, today):
    ...
    lines = [f"🧊 Machines: {len(records)} total, {running_count} currently running", ""]
...
def _handle_electricity_question(question, today):
    ...
    if not rows:
        return f"⚡ No electricity bill recorded for {label}."
...
def _handle_expense_breakdown_question(question, today):
    ...
    lines.append("")
    lines.append(
        "ℹ️ Fixed Asset Depreciation is not included here (it's only "
        "shown on the Home dashboard - it isn't a real row in expenses)."
    )
    return "\n".join(lines)
...
def _handle_plastic_question(question, today):
    ...
    verb_label = "Used" if wants_usage else "Purchased"

    rows = _plastic_entries_in_range(node, date_from, date_to, is_all_time)
    if not rows:
        return f"🛍️ No plastic {verb_label.lower()} recorded for {label}."
    ...
    if node == "plastic_purchases":
        lines.append(f"💰 Total Cost: {_peso(total_peso)}")
    lines.append("")
...
def _handle_expenses_question(question, today):
    ...
    if _PLASTIC_INTENT.search(q) and _SALES_BASED_INTENT.search(q):
        actual = _handle_plastic_question("plastic used " + question, today)
        return (
            "⚠️ There isn't yet a safe way to calculate plastic usage "
            "based on your sales - packaging varies depending on the order's "
            "mode and kg size (it's not always 1 order = 1 bag), so guessing "
            "at a ratio could easily give a wrong estimate.\n\n"
            "Here's the ACTUAL recorded plastic usage instead:\n\n"
            + actual +
            "\n\nIf you'd still like a sales-based estimate, just tell me "
            "what ratio/formula to use (e.g. \"always 1 order = 1 5Kg bag\") "
            "and it can be coded in."
        )
...
def _handle_custom_query(entry, question, today):
    ...
    if query_type == "canned":
        ...
        if not answer:
            return ("⚠️ A custom query matching your question was found, "
                     "but it has no answer set - please edit it in the "
    ...
    return (
        f"⚠️ You've registered a custom query of type '{query_type}', "
        "but this version of Ask AI doesn't support it yet (only v1 "
        "canned-answer queries work for now). Ask Claude to add "
        "report-type custom queries when you're ready to upgrade."
    )
...
def _validate_custom_query_input(body):
    ...
    if not keywords:
        return None, None, "At least one keyword is required."
    if len(keywords) > 10:
        return None, None, "Maximum of 10 keywords per query."
    if any(len(k) < 3 for k in keywords):
        return None, None, "Each keyword must be at least 3 characters (so it isn't too generic)."
    if not answer:
        return None, None, "An answer is required."
    if len(answer) > 2000:
        return None, None, "That answer is too long, please shorten it (max 2000 characters)."
...
def api_custom_queries_update(query_id):
    ...
    if not existing:
        return jsonify({"ok": False, "error": "Could not find this query - it may have been deleted."}), 404
    ...
    try:
        ...
        return jsonify({"ok": False, "error": "Could not save the edit. Please try again."}), 500
...
def api_custom_queries_delete(query_id):
    try:
        ...
        return jsonify({"ok": False, "error": "Could not delete. Please try again."}), 500
...
def api_ai_sales_ask():
    ...
    if not question:
        return jsonify({"ok": False, "error": "You didn't ask anything."}), 400
    if len(question) > 500:
    ...
    if topic != "sales":
        try:
            ...
            return jsonify({
                "ok": False,
                "error": "There was an error retrieving data for this. Please try again.",
            }), 500
    ...
    if not parsed or not parsed.get("is_sales_question"):
        ...
        return jsonify({
            "ok": True,
            "reply": "I couldn't understand what you're asking. Try something like:\n"
                     "  • \"total sales last May\" / \"total sales this week\"\n"
                     "  • \"credit balance for [store name]\" / \"list of resellers\"\n"
                     "  • \"who has the most loyalty points\" / \"what rewards are available\"\n"
                     "  • \"who is active staff\"\n"
                     "  • \"which machines are overdue for PM\" / \"total harvest this week\"\n"
                     "  • \"electricity bill last month\" / \"expense breakdown this year\"\n"
                     "  • \"how many plastic bags were purchased last month\"\n\n"
                     "Tip: if you keep asking this, register it in the "
                     "\"⚙️ Custom Queries\" section above so it gets a fixed answer "
    ...
    if not is_all_time and not date_from and not date_to:
        return jsonify({
            "ok": True,
            "reply": "I couldn't understand which date range you meant. "
                     "Please try again with something more specific (e.g. \"sales in May 2026\" or \"this week\").",
        })
...
#voiceReplyBtn{font-size:11px;padding:6px 12px;border-radius:20px;border:1px solid #ccd;background:#fff;color:#00609C;cursor:pointer;font-weight:700}
#voiceReplyBtn.active{background:#22c55e;color:#fff;border-color:#22c55e}
#voiceReplyBtn:disabled{opacity:.35;cursor:not-allowed}
...
<div class="card">
  ...
  <div class="inputRow">
    <input type="text" id="questionInput" placeholder="e.g. how much did I sell last May?" autocomplete="off">
  ...
  <div class="voiceRow">
    ...
    <button id="voiceReplyBtn" type="button">🔊 Voice Reply</button>
    <span id="handsFreeStatus">🎧 ON — say "OMG" followed by your question (e.g. "OMG how much did I sell today")</span>
  </div>
  <div class="examples">
    <div class="ex-hint">👆 White chip = sends right away. Yellow/dashed chip = fills the input only (replace [store name] before sending). Tap a category name to open/close it.</div>
    ...
    <div class="ex-group">
      ...
      <div class="ex-group-body">
        <span onclick="sendExample(this)">how much did I sell last May?</span>
        ...
        <span onclick="sendExample(this)">who is my top reseller this month?</span>
        <span onclick="sendExample(this)">all time sales</span>
        <span onclick="sendExample(this)">sales report for September 2026</span>
        <span onclick="sendExample(this)">how many transactions this week?</span>
      </div>
    </div>
    ...
    <div class="ex-group">
      ...
      <div class="ex-group-body">
        <span onclick="sendExample(this)">list of resellers</span>
        <span onclick="sendExample(this)">who has the biggest credit balance?</span>
        <span class="fill" onclick="fillExample(this)">credit balance for [store name]?</span>
        <span class="fill" onclick="fillExample(this)">price per kg for [store name]?</span>
        <span class="fill" onclick="fillExample(this)">last order for [store name]?</span>
        <span class="fill" onclick="fillExample(this)">how many times has [store name] ordered this month?</span>
        <span class="fill" onclick="fillExample(this)">how much profit did [store name] make?</span>
        <span class="fill" onclick="fillExample(this)">contact number of [store name]?</span>
      </div>
    </div>
    ...
    <div class="ex-group">
      ...
      <div class="ex-group-body">
        <span onclick="sendExample(this)">who has the most loyalty points?</span>
        <span onclick="sendExample(this)">what rewards are available?</span>
        <span class="fill" onclick="fillExample(this)">how many points does [store name] have?</span>
        <span class="fill" onclick="fillExample(this)">how many times has [store name] redeemed a reward?</span>
        <span onclick="sendExample(this)">show the reward catalog</span>
      </div>
    </div>
    ...
    <div class="ex-group">
      ...
      <div class="ex-group-body">
        <span onclick="sendExample(this)">who is active staff?</span>
        <span onclick="sendExample(this)">list all staff</span>
        <span onclick="sendExample(this)">list all employees</span>
      </div>
    </div>
    ...
    <div class="ex-group">
      ...
      <div class="ex-group-body">
        <span onclick="sendExample(this)">which machines are overdue for PM?</span>
        <span onclick="sendExample(this)">total harvest this week?</span>
        <span onclick="sendExample(this)">list all machines</span>
        <span onclick="sendExample(this)">list all equipment</span>
      </div>
    </div>
    ...
    <div class="ex-group">
      ...
      <div class="ex-group-body">
        <span onclick="sendExample(this)">electricity bill this month?</span>
        <span onclick="sendExample(this)">electricity summary this year</span>
        <span onclick="sendExample(this)">electricity summary last year</span>
        <span onclick="sendExample(this)">monthly breakdown of electricity bills this year</span>
        <span onclick="sendExample(this)">expense breakdown this month</span>
        <span onclick="sendExample(this)">how many plastic bags were purchased this month?</span>
        <span onclick="sendExample(this)">how many plastic bags were used this month?</span>
      </div>
    </div>
  </div>
</div>
...
<div class="card cq-card">
  <div class="cq-header" onclick="toggleCustomQueries()">
    ...
    <span class="cq-toggle" id="cqToggleIcon">Show ▼</span>
  </div>
  <div id="cqPanel" style="display:none">
    <p class="cq-hint">
      Register your own question + answer pairs here - when the system can't
      understand a question (or you just want your own fixed answer), add it
      here. Example: keywords = "price of diesel, how much is diesel",
      answer = "Our diesel price is currently ₱65/liter."
      <br><br>
      ⚠️ The answer here is fixed/static text - it does NOT pull live
      numbers from the database (unlike the sales/expenses/etc. above). If
      the real answer changes, you'll need to edit it here again.
    </p>

    <div id="cqList"><p class="cq-empty">Tap "Show" to load.</p></div>

    <div class="cq-form">
      <label>Keywords (comma-separated, 3+ letters each)</label>
      <input type="text" id="cqKeywords" placeholder="e.g. price of diesel, how much is diesel">
      <label>Answer</label>
      <textarea id="cqAnswer" rows="3" placeholder="Type the answer you want the system to give"></textarea>
      <div class="cq-form-actions">
        <button id="cqSaveBtn" onclick="saveCustomQuery()">➕ Save</button>
...
// ---------------------------------------------------------------------
// Voice REPLY / text-to-speech (Sept 26 2026, ISESMO request: "gusto ko
// na voice din ang sagot ng ai") - speaks every real bot answer out loud
// using the browser's built-in Web Speech API (speechSynthesis), the
// same API already proven working elsewhere in this system (the Live
// Orders "New Order" voice alarm). Default ON since that's what was
// explicitly asked for, but still toggleable via the 🔊 Voice Reply
// button in case ISESMO wants it quiet sa ibang oras - the choice is
// remembered in localStorage so it survives a page reload.
//
// IMPORTANT edge case this handles: when Hands-Free voice INPUT is also
// on, the mic is continuously listening. If we let the AI's own spoken
// reply play while the mic is still listening, the mic can pick up the
// TTS audio itself as if ISESMO said it - a feedback loop that can even
// mishear the wake word "OMG" in the AI's own voice and mis-fire a new
// question. So: pause the hands-free mic while speaking, and only resume
// listening once the reply finishes (utter.onend below).
// ---------------------------------------------------------------------
const voiceReplyBtn = document.getElementById('voiceReplyBtn');
const SPEECH_SYNTH_SUPPORTED = ('speechSynthesis' in window);
let voiceReplyEnabled = true;
try {
  const saved = localStorage.getItem('omega_ai_voice_reply');
  if(saved !== null) voiceReplyEnabled = (saved === 'true');
} catch(e){ /* localStorage unavailable (private mode, etc.) - keep default */ }

if(!SPEECH_SYNTH_SUPPORTED){
  voiceReplyBtn.disabled = true;
  voiceReplyBtn.title = 'Voice output is not supported in this browser - please use Chrome.';
  voiceReplyEnabled = false;
} else {
  updateVoiceReplyBtnUI();
  voiceReplyBtn.addEventListener('click', function(){
    voiceReplyEnabled = !voiceReplyEnabled;
    try { localStorage.setItem('omega_ai_voice_reply', String(voiceReplyEnabled)); } catch(e){}
    updateVoiceReplyBtnUI();
    if(!voiceReplyEnabled) window.speechSynthesis.cancel();
  });
}

function updateVoiceReplyBtnUI(){
  voiceReplyBtn.classList.toggle('active', voiceReplyEnabled);
  voiceReplyBtn.textContent = voiceReplyEnabled ? '🔊 Voice Reply (ON)' : '🔇 Voice Reply';
}

// Strips emoji/icon characters out of a reply before it's spoken aloud
// (Sept 26 2026, ISESMO request: "icons should no longer be read aloud") -
// the icons still show visually in the chat bubble (addMessage() renders
// the ORIGINAL text, untouched), this only affects what speakReply() feeds
// into SpeechSynthesisUtterance. Uses a broad Unicode range sweep instead
// of a hardcoded list of only the specific icons currently in this file,
// so it keeps working if more emoji get added to a reply string later
// without needing another code change here.
...
function speakReply(text){
  if(!SPEECH_SYNTH_SUPPORTED || !voiceReplyEnabled || !text) return;
  try {
    const spokenText = stripEmojiForSpeech(text);
    if(!spokenText) return; // nothing left to say after stripping icons (rare, but don't speak an empty utterance) - mic was never paused, so nothing to resume either
    window.speechSynthesis.cancel(); // avoid overlapping with the next reply
    // Pause hands-free listening while the AI talks, if it's ON -
    // avoids a feedback loop (mic hearing the AI's own voice).
    const wasHandsFreeListening = (typeof handsFreeEnabled !== 'undefined' && handsFreeEnabled);
    if(wasHandsFreeListening){
      suppressHandsFreeRestart = true;
    ...
    // Replies are English now (Sept 26 2026) - prefer an installed 'en-US'
    // voice, but fall back to ANY 'en' voice (e.g. 'en-GB', 'en-AU') in
    // case the device doesn't have en-US specifically installed, same
    // graceful-fallback pattern as the old fil-PH lookup this replaced.
    const enVoice = voices.find(v => v.lang.toLowerCase() === 'en-us') || voices.find(v => v.lang.toLowerCase().includes('en'));
    if(enVoice) utter.voice = enVoice;
    utter.onend = function(){
      if(wasHandsFreeListening && handsFreeEnabled){
        suppressHandsFreeRestart = false;
        try { voiceRecognition.start(); } catch(e){ /* already restarting via its own onend cycle */ }
      }
    };
    ...
    console.log('speakReply error', e);
  }
}
...
function addMessage(text, cls){
  ...
  if(cls.indexOf('bot') !== -1 && cls.indexOf('loading') === -1){
    ...
    // Auto-speak every real bot answer (not the loading bubble) - this is
    // the actual "voice reply" feature; speakReply() itself checks the
    // voiceReplyEnabled toggle, so this call is always safe to make.
    speakReply(text);
  }
...
async function askQuestion(){
  ...
  const loadingEl = addMessage('Checking the data...', 'bot loading');
  ...
  try {
    ...
    if(data.ok){
      ...
      addMessage('⚠️ ' + (data.error || 'An unexpected error occurred.'), 'bot error');
    }
  } catch(e){
    loadingEl.remove();
    addMessage('⚠️ Could not reach the server. Check your connection.', 'bot error');
  } finally {
...
// Sept 26 2026 (voice reply feature): when true, the normal hands-free
// auto-restart-on-end loop below skips restarting the mic - set while
// the AI's spoken reply (speakReply(), above) is playing, so the mic
// doesn't pick up its own voice output as a new "OMG" command. Cleared
// again once the reply finishes (utter.onend/onerror in speakReply()),
// which explicitly restarts listening itself at that point.
let suppressHandsFreeRestart = false;

if(!SpeechRecognitionAPI){
  micBtn.disabled = true;
  micBtn.title = 'Voice input is not supported in this browser - please use Chrome.';
  handsFreeBtn.disabled = true;
  handsFreeBtn.title = 'Voice input is not supported in this browser - please use Chrome.';
} else {
  ...
  voiceRecognition.onerror = function(event){
    if(event.error === 'not-allowed' || event.error === 'service-not-allowed'){
      addMessage('⚠️ Microphone access was not allowed. Please allow it in your browser settings first.', 'bot error');
    ...
    if(voiceMode === 'handsfree'){
      ...
      if(handsFreeErrorStreak >= HANDS_FREE_MAX_ERROR_STREAK){
        addMessage('⚠️ Hands-Free listening keeps erroring (' + event.error + '). Turning it off for now - tap Hands-Free again once the mic is ok.', 'bot error');
        stopHandsFree();
      }
      return;
    }
    if(event.error !== 'no-speech' && event.error !== 'aborted'){
      addMessage('⚠️ There was a problem with voice input (' + event.error + '). Please try again.', 'bot error');
    }
  };
  ...
  voiceRecognition.onend = function(){
    ...
    if(voiceMode === 'handsfree' && handsFreeEnabled){
      if(suppressHandsFreeRestart) return; // AI is talking - speakReply()'s onend will restart listening itself
      handsFreeRestartTimer = setTimeout(function(){
        if(!handsFreeEnabled || suppressHandsFreeRestart) return;
        try {
  ...
  micBtn.addEventListener('click', function(){
    ...
    try {
      ...
      addMessage('⚠️ Could not start voice input. Please try again.', 'bot error');
    }
  });
  ...
  document.addEventListener('visibilitychange', function(){
    if(document.hidden){
      ...
      if(handsFreeEnabled){
        stopHandsFree();
        addMessage('🎧 Hands-Free was turned off because you left the tab/app. Tap it again when you\\'re back.', 'bot');
      }
    }
  });
}
...
function startHandsFree(){
  ...
  handsFreeBtn.textContent = '🎧 ON (tap to turn OFF)';
  handsFreeStatus.style.display = 'block';
  questionInput.value = '';
  try {
    voiceRecognition.start();
  } catch(e){
    addMessage('⚠️ Could not start Hands-Free listening. Please try again.', 'bot error');
    stopHandsFree();
  }
}
...
function toggleCustomQueries(){
  ...
  cqToggleIcon.textContent = willShow ? 'Hide ▲' : 'Show ▼';
...
async function loadCustomQueries(){
  ...
  try {
    ...
    const data = await res.json();
    if(!data.ok){
      cqList.innerHTML = '<p class="cq-empty">There was an error loading.</p>';
      return;
    }
    const items = data.items || [];
    document.getElementById('cqCount').textContent = items.length ? `(${items.length})` : '';
    if(!items.length){
      cqList.innerHTML = '<p class="cq-empty">You haven\\'t registered any custom queries yet.</p>';
      return;
    }
    cqList.innerHTML = '';
    items.forEach(item => {
    ...
    cqList.innerHTML = '<p class="cq-empty">Could not reach the server.</p>';
  }
}
...
async function saveCustomQuery(){
  ...
  if(!kwRaw || !answer){
    alert('Keywords and an answer are required.');
    return;
  }
...
addMessage('Hi! You can ask me about your sales, resellers, loyalty points/rewards, staff, machines, or expenses (including electricity and plastic). I\\'ll answer straight from your real records. Just tap the chips below - white ones send right away, yellow/dashed ones only fill in the input so you can replace [store name] first. There\\'s also a "⚙️ Custom Queries" section below where you can add your own question + answer pairs.', 'bot');
</script>
</body>
</html>
"""

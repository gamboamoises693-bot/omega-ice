...
# Model history for this project (keep this updated - Google retires Gemini
# model names periodically and a retired model returns a 404 that LOOKS
# identical to a config/URL bug):
#   gemini-2.5-flash -> access-restricted for new API keys (Sept 2026)
#   gemini-2.0-flash -> fully retired, 404 NOT_FOUND (confirmed via Render
#                       logs, Sept 25 2026): "This model models/gemini-2.0
#                       -flash is no longer available. ... use models/
#                       gemini-3.8-flash"
#   gemini-3.8-flash  -> current GA model as of Sept 2026 (this default)
# If Ask Sales ever starts 404'ing again, it is very likely this - check
# https://ai.google.dev/gemini-api/docs/latest-model for the current model
# name before assuming the code broke, and override via the GEMINI_MODEL
# env var on Render (no redeploy needed) while updating this default.
# (The full request URL is built inline in _ask_gemini_for_date_range, as a
# plain f-string, with the API key sent via requests' `params=` instead of
# being baked into the string - see the comment there for why.)
...
# instant (no network round-trip to Gemini) and immune to Gemini being
# overloaded (503) or quota-exhausted (429, free tier caps at 20 req/min -
# see _ask_gemini_for_date_range's retry comment). The AI is now only
# actually called for genuinely freeform/ambiguous questions it's needed
# for. Mirrors the exact same date-range RULES given to Gemini in the
# prompt below - if you change one, change the other.
# ---------------------------------------------------------------------------
_MONTH_NAMES = {
...
def _try_local_date_extraction(question, today):
    """Returns the same dict shape _ask_gemini_for_date_range's parsed JSON
    has, or None if the question isn't a confident match for one of these
    fixed patterns (caller should fall through to Gemini in that case)."""
    ...
    return None  # not a confident local match - let Gemini handle it
...
Question: {question}"""
    ...
    # Built as a plain f-string (no .format()/template indirection) and the
    # API key is passed via `params=` instead of being concatenated into the
    # URL string, so requests/urllib3 does the query-string encoding - this
    # removes every layer that could plausibly mangle the URL between here
    # and the actual HTTP request.
    api_url = f"https://generativelanguage.googleapis.com/v1beta/models/{GEMINI_MODEL}:generateContent"
    payload = {
        "contents": [{"parts": [{"text": prompt}]}],
        # `temperature` is deprecated on gemini-3.8-flash (Google's current
        # guidance, Sept 2026) - `thinking_level` is its replacement.
        # "low" is correct here: this call is a single-shot JSON extraction
        # (which calendar dates does the question mean?), not something
        # that benefits from multi-step reasoning, so "low" both matches
        # the task and keeps latency/cost down - a small help against
        # landing in an overloaded serving window (see the 503 investigation,
        # Sept 25-26 2026), though it does not fix 503 on its own.
        "generationConfig": {"thinking_level": "low", "responseMimeType": "application/json"},
    }

    # Retry ONLY on 503 ("model overloaded" - genuinely transient, often
    # clears within seconds). Deliberately NOT retrying on 429: Gemini's own
    # error body tells you exactly how long to wait ("Please retry in 53s"),
    # and the free tier's generate_content_free_tier_requests quota is tiny
    # (confirmed via Render logs, Sept 25 2026: limit=20). A 1.5s backoff
    # retry against a 20-request quota does not help - it just spends a
    # SECOND request from that same tiny quota chasing the first, which is
    # exactly what caused six back-to-back 429s in one test session. Also
    # not retrying on a timeout/connection error: that attempt already
    # burned its own timeout budget, and gunicorn kills this whole worker if
    # ONE request handling takes longer than ~30s (the WORKER TIMEOUT bug
    # from Sept 25, 2026 - see _manila_today's history). Budget here stays
    # well under that: 2 attempts x 12s timeout + a 1.5s backoff = 25.5s
    # worst case.
    last_exc = None
    resp = None
    for attempt in (1, 2):
        try:
            resp = requests.post(api_url, params={"key": api_key}, json=payload, timeout=12)
    ...
    if last_exc is not None or resp is None or not resp.ok:
        ...
        # Kept permanently (not temp debug): Google retires Gemini model
        # names periodically, and the resulting 404's real cause only shows
        # up in the response BODY (e.g. "model no longer available, use
        # models/gemini-X"), never in str(e) alone. Logging status+body here
        # means the next retirement is a 2-minute Render Logs check instead
        # of the multi-hour investigation this one took (Sept 25, 2026).
        print(
            f"[ai-sales] Gemini request failed: status={status} body={(body or '')[:500]!r}",
            flush=True,
        )
        if status == 429:
            return None, "Naabot na ang free quota ng AI ngayon (limited requests per minute). Sandali lang, subukan ulit pagkalipas ng isang minuto."
        if status == 503:
            return None, "Sobrang busy ang AI service ngayon (parte ito ng Gemini, hindi sa app natin). Subukan ulit pagkalipas ng ilang segundo."
...
def api_ai_sales_ask():
    ...
    # atbp). Zero latency, zero Gemini quota usage, immune sa 503/429. Kung
    # hindi ma-match (freeform/ambiguous question), saka pa lang tatawagin
    # si Gemini.

...
_EXPENSE_KEYWORDS = re.compile(
    r"\b(expenses?|gastos|electricity|kuryente|koryente|electric ?bill|"
    r"ilaw|consumables?|fuel|gasolina|diesel|maintenance|plastic|"
    r"packaging|bags?)\b", re.I
)

# Sub-intents WITHIN the expenses topic.
_ELECTRICITY_INTENT = re.compile(
    r"\b(electricity|kuryente|koryente|electric ?bill|ilaw)\b", re.I
)
...
def _handle_electricity_question(question, today):
    """Covers both "Ilang electricity bill ko last xxxx" and "Summary ng
    electricity ko this year/last year" - one richer reply answers both
    phrasings (bill count + total ₱ covers the first; + total kWh and
    average real ₱/kWh covers the second)."""
    date_from, date_to, is_all_time, label = _resolve_question_range(question, today)
...
.ex-category{font-size:11px;color:#00609C;font-weight:700;margin:10px 0 3px;opacity:.75}
.ex-category:first-of-type{margin-top:4px}
...
<div class="card">
  ...
  <div class="examples">
    <div class="ex-hint">👆 Puting chip = direktang magse-send. Yellow/dashed chip = i-fill lang (palitan ang [store name] bago i-send).</div>

    <div class="ex-category">📊 Sales</div>
    ...
    <div class="ex-category">🏪 Resellers</div>
    ...
    <div class="ex-category">🏆 Loyalty &amp; Rewards</div>
    ...
    <div class="ex-category">💸 Expenses</div>
    ...
    items.forEach(item => {
      ...
      div.innerHTML =
        ...
        '<div class="cq-item-actions">' +
          '<button onclick="startEditCustomQuery(\'' + item.id + '\')">✏️ Edit</button>' +
          '<button class="danger" onclick="deleteCustomQuery(\'' + item.id + '\')">🗑️ Delete</button>' +
        '</div>';
